...
def main():
    ...
    print("Table with out of sample errors for k = 3,5,7,9,11:")
    k_values =[3,5,7,9,11]
    out_of_sample_error = results
    print("k value", '\t', "error")
    print("-------", '\t', "----------------")
    for i in range(5):
    	print(k_values[i], '\t', out_of_sample_error[i])
    ...
    k = [3,5,7,9,11]
    pyplot.bar(k, results)
    pyplot.ylabel("out of sample error")
    pyplot.xlabel("value of k")
    pyplot.title("Out of sample error for k = 3,5,7,9,11")
    pyplot.show()
