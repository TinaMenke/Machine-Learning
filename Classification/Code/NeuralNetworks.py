...
def best_hyperparams():
    ...
    return params["hidden_layer_sizes"]
...
def create_model(train_x, train_y, hyperparams):
    ...
    classifier = None
    classifier = MLPClassifier(hidden_layer_sizes=hyperparams, random_state = 7)
...
def main():
    ...
    print("Table with out of sample errors for hidden layer sizes 5, 40 and 70.")
    out_of_sample_error = results

    print("layer size", '\t', "error")
    print("-----", '\t', "-----")
    for i in range(3):
    	print(hidden_layer_size[i], '\t', results[i])
    ...
    layer_size=[5,40,70]
    pyplot.bar(layer_size, results)
    pyplot.ylabel("out of sample error")
    pyplot.xlabel("hidden layer size")
    pyplot.title("Out of sample error vor hidden layer sizes 5, 40 and 70.")
    pyplot.show()
